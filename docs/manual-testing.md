...
- [ ] **Devices ▸ Record screen** — 처음 누르면 화면 녹화 권한을 물어본다.
      허용 후 다시 눌러야 시작되는 경우가 있다
- [ ] 녹화 중 메뉴 바 아이콘이 **빨간 점**으로 바뀌는가
- [ ] 10초쯤 말하고 사운드보드도 한 번 눌러본 뒤 정지 → **Show in Finder**
- [ ] 파일을 QuickTime으로 열어서: 화면이 보이는가, **변조된 목소리**가 들리는가,
      사운드보드 소리도 함께 들어갔는가
- [ ] **소리와 화면이 어긋나지 않는가** (길게 녹화할수록 드러난다 — 1분 이상 해볼 것)
- [ ] 모니터링(내 목소리 듣기)을 켠 채 녹화해도 파일에 **에코가 생기지 않는가**
      — 녹화되는 것은 내보낸 믹스이지 헤드폰으로 나가는 것이 아니다
